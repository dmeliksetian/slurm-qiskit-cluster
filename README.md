...
./setup/00-check-prereqs.sh                  # verify host dependencies
./setup/00b-configure-system.sh              # detect hardware, generate config files
...
│   ├── docker-compose.yml       # generated by 00b-configure-system.sh (gitignored)
│   ├── docker-compose.yml.template  # tracked template with @@PLACEHOLDER@@ tokens
│   ├── build-all.sh             # runs inside builder container
│   └── config/                  # Slurm config files
│       ├── slurm.conf           # generated by 00b-configure-system.sh (gitignored)
│       ├── slurm.conf.template  # tracked template
│       ├── gres.conf            # generated by 00b-configure-system.sh (gitignored)
│       ├── gres.conf.template   # tracked template
│       ├── plugstack.conf
...
└── setup/
    ├── 00-check-prereqs.sh
    ├── 00b-configure-system.sh  # detects hardware, renders config files from templates
    ├── 01-build-images.sh
    ├── 02-build-shared.sh
